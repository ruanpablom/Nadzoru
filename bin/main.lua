...
safeload('luaxml2es','luaxml2es',true,[[Foo Bar da Silva]])
...
function Controller:build()
    ...
    --File
    self.gui:add_action('workspace_save'   , "_Save Workspace", "Save Workspace", nil, self.save_workspace, self)
    self.gui:add_action('workspace_load'   , "_Load Workspace", "Load Workspace", nil, self.load_workspace, self)
    ...
    self.gui:add_action('automata_clone'        , "_Clone Automaton", "Create a Copy of an Automaton", nil, self.clone_automaton, self)
    self.gui:add_action('automata_remove'       , "_Remove Automaton", "Remove an Automaton", nil, self.remove_automaton, self)
    ...
    self.gui:add_action('automata_import_tct', "_Import TCT Automaton", "Import a TCT (.ADS) automaton file", nil, self.import_tct, self)
	self.gui:add_action('automata_import_mp', "_Import Mission Planner", "Import a Mission (.TXT)  file", nil, self.import_mp, self)
	...
	self.gui:add_action('operations_create_sub_mission', "Create Sub-Mission", "Create the Submissions For Mission planner", nil, self.operations_create_sub_mission,self)
    ...
    self.gui:add_action('operations_join_non_coaccessible', "_Join Non Coaccessible", "Join Non Coaccessible States", nil, self.operations_join_non_coaccessible, self)
    ...
    self.gui:add_action('operations_synchronization', "_Synchronization", "Synchronization of two or more automata", nil, self.operations_synchronization, self)
	...
	self.gui:add_action('operations_observer', "_Observer", "Calculate the observer of one automaton", nil, self.operations_observer, self)
	...
	self.gui:add_action('operations_RemoveAfterFailure', "_Remove After Failure", "Remove States After Failure", nil, self.operations_raf, self)
    ...
    -- ** Menu-Action Link ** --
    --File
    self.gui:append_menu_item('file','workspace_save')
    self.gui:append_menu_item('file','workspace_load')
    self.gui:append_menu_item('file','tab_close_current')
    self.gui:append_menu_separator('file')
    self.gui:append_menu_item('file', 'quit')
    ...
    self.gui:append_menu_item('automata','automata_remove')
    self.gui:append_menu_item('automata','automata_clone')
    self.gui:append_menu_item('automata','automata_import_ides')
    self.gui:append_menu_item('automata','automata_import_tct')
	self.gui:append_menu_item('automata','automata_import_mp')
    ...
    self.gui:append_menu_item('automata','automata_group_remove')
    self.gui:append_menu_separator('automata')
	...
	self.gui:append_menu_item('operations','operations_complement')
  ...
  self.gui:append_menu_item('operations','operations_safe_diagnoser')
	...
	self.gui:append_menu_item('operations','operations_observer')
	self.gui:append_menu_item('operations','operations_product')
	self.gui:append_menu_item('operations','operations_projection')
	self.gui:append_menu_item('operations','operations_RemoveAfterFailure')
    ...
    self.gui:append_sub_menu('operations','problems', "Problems")
    self.gui:append_menu_item('problems','operations_check_all')
    self.gui:append_menu_separator('problems')
    self.gui:append_menu_item('problems','operations_check_choice_problem')
    self.gui:append_menu_item('problems','operations_check_avalanche_effect')
    self.gui:append_menu_item('problems','operations_check_inexact_synchronization')
    self.gui:append_menu_item('problems','operations_check_simultaneity')
    self.gui:append_menu_item('automata', 'simulategraphviz')
	self.gui:append_menu_separator('operations')
	self.gui:append_menu_item('operations','operations_create_sub_mission')
...
local space_things = {
	...
	['TXT'] = { name = 'Mission Planner'        , class = Automaton     , load = 'MP_import' 			  },
...
-- Imports MP-WP
function Controller.import_mp( data )
    local dialog = gtk.FileChooserDialog.new(
        "Select the file", nil, gtk.FILE_CHOOSER_ACTION_OPEN,
        "gtk-cancel", gtk.RESPONSE_CANCEL,
        "gtk-ok", gtk.RESPONSE_OK
    ...
    filter:set_name("Mission Planner")
    dialog:add_filter(filter)
    dialog:set("select-multiple", true)
    local response = dialog:run()
    dialog:hide()
    ...
    if response == gtk.RESPONSE_OK and filenames then
        for k_filename, filename in ipairs( filenames ) do
            local new_automaton = Automaton.new( data.param )
            new_automaton:MP_import( filename )
            data.param:element_add( new_automaton )
            Controller.add_events_from_automaton( data.param, new_automaton)
            Controller.create_automaton_tab( data, new_automaton ) --start editing automaton
        end
...
function Controller.code_gen_dfa( data )
    ...
    Selector.new({
        ...
        success_fn = function( results, numresult )
            ...
            local event_map      = #event_map_file>0
            if #automata > 0 and #file_name > 0 and device_id then
                ...
                local cg = CodeGen.new{
                    ...
                    workspace_events = data.param.events,
                }
                if cg then
                    cg:execute( data.gui )
                end
    ...
    :add_file{
        text = "Code File:",
        -- filter = 'c', it's being added at Devices['schneider'].file_extension	= 'xst'
        -- filter_name = "C Code",
    }
    :add_combobox{
...
--@see Selector:run
--@see Controller.create_automaton_tab
function Controller.operations_join_non_coaccessible( data )
     Selector.new({
        title = 'nadzoru',
        success_fn = function( results, numresult )
            local automaton = results[1]
...
---Creates the observer automaton of chosen automata.
--TODO
--@param data TODO
--@see Automaton:observer
--@see Selector:add_multipler
--@see Selector:run
--@see Controller.create_automaton_tab
function Controller.operations_observer( data )
	Selector.new({
		title                       = 'nadzoru',
		success_fn                  = function( results, numresult )
			local automaton         = results[1]
			if automaton then
				local new_automaton = automaton:observer(data,automaton)
				if new_automaton and not keep then
					new_automaton:set('file_name', 'observer(' .. automaton:get('file_name') .. ')')
					data.param.elements:append( new_automaton )
...
function Controller.operations_projection( data )
	...
	Selector.new({
		...
		success_fn = function( results, numresult )
            local automaton = results[1]
            if automaton then
				automaton:projection( data,automaton)
            end
        end,
    })
   :add_combobox{
	...
	Selector.new({
        ...
        success_fn = function( results, numresult )
			...
			if automaton then
				...
				if new_automaton then
				new_automaton:set( 'file_name', 'diagnoser' )
	            data.param.elements:append( new_automaton )
	            Controller.create_automaton_tab( data, new_automaton ) --start editing automaton
				end
            end
...
function Controller.operations_raf( data )
    Selector.new({
        title = 'nadzoru',
        success_fn = function( results, numresult )
            local automaton = results[1]
            if automaton then
                local new_automaton = automaton:raf( false )
                if new_automaton then
	                new_automaton:set('file_name', 'deterministic(' .. automaton:get('file_name') .. ')')
	                data.param.elements:append( new_automaton )
    	            Controller.create_automaton_tab( data, new_automaton ) --start editing automaton
...
---Creates a sub-mission for mission planner
function Controller.operations_create_sub_mission( data )
     Selector.new({
        title = 'nadzoru',
        success_fn = function( results, numresult )
            local automaton = results[1]
            if automaton then
            automaton:create_sub_mission( true )
            end
        end,
    })
    :add_combobox{
