...
function DrawNetwork (estrutura --[[FBlist, inst, origem]])
    --Containers
    local Comp     = clone(estrutura, true)
    local window   = gtk.Window.new()
    local main_box = gtk.Box.new(gtk.ORIENTATION_VERTICAL, 5)
    local area     = gtk.DrawingArea.new()
    area:set_size_request(3000, 3000)
    ...
    local paned    = gtk.Paned.new(gtk.ORIENTATION_HORIZONTAL)
    local scroll   = gtk.ScrolledWindow.new()
    scroll:set_policy(true, true)
    local sidebox  = gtk.Box.new(gtk.ORIENTATION_VERTICAL, 0) --sera editado dinamicamente
    ---combo box
    ...
    local insert = true
    local N, draw_type = 1, " "
    local FBlist, inst, Origem = nil, nil, nil
    FBlist, inst, Origem = {}, {}, {}
    local function initiate()
        if radio2:get_active() then
            draw_type = 'block'
        elseif radio1:get_active() and Comp.Class =='Basic' then
            draw_type = 'ECC'
        else
            draw_type = 'network'
        end
        --Extrai do CompFB, FBlist, inst e origem
        FBlist, inst, Origem = nil, nil, nil
        FBlist, inst, Origem = {}, {}, {}
    ...
    initiate()    
    local function draw_network(widget, cr)
        ...
        if insert then
            ...
            --------------------------------------------------------------
                ...
                local function add_input_event (label)
                ...
                local function add_output_event (label)
    ...
    local function draw_block(widget, cr)
        --conta quantos eventos e variaveis
        if insert then
            for i, v in ipairs (Comp.FBNetwork) do
                combo:insert_text(0, v[1])
                combo:set_active(0)
            end
        insert = false
        end
        ...
        local function add_input_var (label)
            local pos = {iactvar[1], iactvar[2] + 20*N}
            slot.ivar = slot.ivar + 1
        ...
        local function add_output_var (label)
            local pos = {oactvar[1], oactvar[2] + 20*N}
            slot.ovar = slot.ovar + 1
    ...
    local function draw_ECC (widget, cr)
        local function draw_arrow(point, th)
            cr:set_line_width(1.5*N)
            cr:set_source_rgb(0, 0, 0)
            cr:move_to(point[1], point[2])
            cr:rel_line_to(-12*math.cos(th)*N +8*math.sin(th)*N, -12*math.sin(th)*N -8*math.cos(th)*N)
            cr:move_to(point[1], point[2])
            cr:rel_line_to(-12*math.cos(th)*N -8*math.sin(th)*N, -12*math.sin(th)*N +8*math.cos(th)*N)
            cr:stroke()
            return false
        ...
        local cond_pos = 0
        local st_pos   = 0
        local how_many   = {}     --how_many[st1] = {top, down, right, left}
        local ocup       = {}     --ocup[st1] = {top, down, right, left} --quantos slots do total ja foram usados
        for i, v in pairs(Comp.stlist) do
            how_many[i] = {0, 0, 0, 0}
            ocup[i]     = {1, 1, 1, 1}
        end
        local transition = {}
        
        local function count(st1, st2, x1, y1, x2, y2)  --x, y source  , x, y destination
            --print(st1, st2)
            --1º Caso - Destino acima e a esquerda do source
            if x1 >=x2 and y1 >= y2 then
                local th = math.atan ((y1-y2)/(x1-x2))
                if th > math.pi/5 then 
                    how_many[st1][1] = how_many[st1][1] + 1  -- + 1 no topo
                    how_many[st2][2] = how_many[st2][2] + 1  -- + 1 abaixo
                    --print(st1..' to '..st2..' destino acima e a esquerda, TOP/DOWN')
                    transition[st1..'.'..st2] = 'TD'  --source=top, dest = down
                else
                    how_many[st1][4] = how_many[st1][4] + 1  -- + 1 a esquerda
                    how_many[st2][3] = how_many[st2][3] + 1  -- + 1 a direita
                    --print(st1..' to '..st2..' destino acima e a esquerda, RIGHT/LEFT')
                    transition[st1..'.'..st2] = 'LR' --source=left, dest = right
                end
            --2º Caso - Destino abaixo e a esquerda do source
            elseif x1 >= x2 and y1 <= y2 then
                local th = math.atan ((y2-y1)/(x1-x2))
                if th > math.pi/5 then 
                    how_many[st1][2] = how_many[st1][2] + 1  -- + 1 no topo
                    how_many[st2][1] = how_many[st2][1] + 1  -- + 1 abaixo
                    --print(st1..' to '..st2..' destino abaixo e a esquerda, TOP/DOWN')
                    transition[st1..'.'..st2] = 'DT' --source=down, dest = top
                else
                    how_many[st1][4] = how_many[st1][4] + 1  -- + 1 a esquerda
                    how_many[st2][3] = how_many[st2][3] + 1  -- + 1 a direita
                    --print(st1..' to '..st2..' destino abaixo e a esquerda, RIGHT/LEFT')
                    transition[st1..'.'..st2] = 'LR' --source=left, dest = right
                end
            --3º Caso - Destino acima e a direita do source
            elseif x1 <= x2 and y1 >= y2 then
                local th = math.atan ((y1-y2)/(x2-x1))
                if th > math.pi/5 then 
                    how_many[st1][1] = how_many[st1][1] + 1  -- + 1 no topo
                    how_many[st2][2] = how_many[st2][2] + 1  -- + 1 abaixo
                    --print(st1..' to '..st2..' acima e a direita, TOP/DOWN')
                    transition[st1..'.'..st2] = 'TD' --source=left, dest = right
                else
                    how_many[st1][3] = how_many[st1][3] + 1  -- + 1 a direita
                    how_many[st2][4] = how_many[st2][4] + 1  -- + 1 a esquerda
                    --print(st1..' to '..st2..' acima e a direita, RIGHT/LEFT')
                    transition[st1..'.'..st2] = 'RL' --source=right, dest = left
                end
            --4º Caso - Destino abaixo e a direita do source
            elseif x1 <= x2 and y1 <= y2 then
                local th = math.atan ((y2-y1)/(x2-x1))
                if th > math.pi/5 then 
                    how_many[st1][2] = how_many[st1][2] + 1  -- + 1 no topo
                    how_many[st2][1] = how_many[st2][1] + 1  -- + 1 abaixo
                    --print(st1..' to '..st2..' abaixo e a direita, TOP/DOWN')
                    transition[st1..'.'..st2] = 'DT' --source=down, dest = top
                else
                    how_many[st1][3] = how_many[st1][3] + 1  -- + 1 a direita
                    how_many[st2][4] = how_many[st2][4] + 1  -- + 1 a esquerda
                    --print(st1..' to '..st2..' abaixo e a direita, RIGHT/LEFT')
                    transition[st1..'.'..st2] = 'RL' --source=right, dest = left
                end
            end
        end
        local n = 1
        local function connect(st1, st2, x1, y1, x2, y2, condition)
            cr:select_font_face('sans', cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
            cr:move_to (50*N, 250*N + 14*cond_pos*N)
            cond_pos = cond_pos + 1
            cr:set_font_size (14*N)
            cr:show_text(st1..' -> '..st2..' Condition: '..condition)
            
            --1º caso TD
            if transition[st1..'.'..st2] == 'TD' then
                cr:move_to (x1*N + (30/how_many[st1][1])*ocup[st1][1]*N, y1*N)
                cr:line_to (x2*N + (30/how_many[st2][2])*ocup[st2][2]*N, y2*N + 40*N)
                cr:stroke()
                --cr:arc(x2 + (30/how_many[st2][2])*ocup[st2][2], y2 + 40, 2, 0, 2*math.pi)
                
                local dx = x2 + (30/how_many[st2][2])*ocup[st2][2] -x1 - (30/how_many[st1][1])*ocup[st1][1]
                local dy = y2+40-y1
                ...
                local pt = {x2*N + (30/how_many[st2][2])*ocup[st2][2]*N, (y2 + 40)*N}
                draw_arrow(pt, th)
                
                cr:set_source_rgb(0.8, 0, 0)
                cr:fill()
                cr:set_source_rgb(0, 0, 0)
                ocup[st1][1] = ocup[st1][1] + 2
                ocup[st2][2] = ocup[st2][2] + 2
                cr:stroke()    
            --2º caso DT
            elseif transition[st1..'.'..st2] == 'DT' then
                
                cr:move_to (x1*N + (30/how_many[st1][2])*ocup[st1][2]*N, y1*N + 40*N)
                cr:line_to (x2*N + (30/how_many[st2][1])*ocup[st2][1]*N, y2*N)
                cr:stroke()
                
                local dx = x2 + (30/how_many[st2][1])*ocup[st2][1] -x1 - (30/how_many[st1][2])*ocup[st1][2]
                local dy = y2-40-y1
                
                local th = math.atan2 (dy , dx)
    
                local pt = {x2*N + (30/how_many[st2][1])*ocup[st2][1]*N, y2*N}
                draw_arrow(pt, th)
                
                cr:set_source_rgb(0.8, 0, 0)
                cr:fill()
                cr:set_source_rgb(0, 0, 0)
                ocup[st1][2] = ocup[st1][2] + 2
                ocup[st2][1] = ocup[st2][1] + 2
                cr:stroke()
            --3º caso RL
            elseif transition[st1..'.'..st2] == 'RL' then
                cr:move_to (x1*N + 60*N, y1*N + (20/how_many[st1][3])*ocup[st1][3]*N)
                cr:line_to (x2*N, y2*N + (20/how_many[st2][4])*ocup[st2][4]*N)
                cr:stroke()
                
                local dx = x2 -x1 - 60
                local dy = y2 + (20/how_many[st2][4])*ocup[st2][4] - y1 - (20/how_many[st1][3])*ocup[st1][3]
                local th = math.atan2 (dy, dx)
                
                local pt = {x2*N, y2*N + (20/how_many[st2][4])*ocup[st2][4]*N}
                draw_arrow(pt, th)
                
                cr:set_source_rgb(0.8, 0, 0)
                cr:fill()
                cr:set_source_rgb(0, 0, 0)
                ocup[st1][3] = ocup[st1][3] + 2
                ocup[st2][4] = ocup[st2][4] + 2
                cr:stroke()
            --4º caso LR
            elseif transition[st1..'.'..st2] == 'LR' then
                cr:move_to (x1*N, y1*N + (20/how_many[st1][4])*ocup[st1][4]*N)
                cr:line_to (x2*N+60*N, y2*N + (20/how_many[st2][3])*ocup[st2][3]*N)
                cr:stroke()
                
                local dx = x2 -x1 + 60
                local dy = y2 + (20/how_many[st2][3])*ocup[st2][3] - y1 - (20/how_many[st1][4])*ocup[st1][4]
                local th = math.atan2 (dy , dx)
                
                local pt = {x2*N+60*N, y2*N + (20/how_many[st2][3])*ocup[st2][3]*N}
                draw_arrow(pt, th)
                
                cr:set_source_rgb(0.8, 0, 0)
                cr:fill()
                cr:set_source_rgb(0, 0, 0)
                ocup[st1][4] = ocup[st1][4] + 2
                ocup[st2][3] = ocup[st2][3] + 2
                cr:stroke()    
            end
        end
        ...
        cr:move_to (50*N, 36*N)
        cr:select_font_face('sans', cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_BOLD)
        cr:set_font_size(16*N)
        cr:show_text('Statelist:')
        for i, v in pairs(Comp.stlist) do
            
            cr:select_font_face('sans', cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
            cr:move_to (50*N, 50*N + 14*st_pos*N)
            st_pos = st_pos + 1
            cr:set_font_size (14*N)
            cr:show_text(i)
            if v.action then
                for g, h in ipairs (v.action) do
                    cr:move_to (55*N, 50*N + 14*st_pos*N)
                    cr:show_text('alg: '..(h.alg or 'N/A')..' ; output: '..(h.out or 'N/A'))
                    st_pos = st_pos + 1
                end
            end
            cr:rectangle (Comp.STdx[i]*N,Comp.STdy[i]*N , 60*N,  40*N)
            cr:move_to (Comp.STdx[i]*N+30*N - 3.3*#i*N ,Comp.STdy[i]*N+22*N)
            cr:select_font_face('sans', cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
            cr:set_font_size(10*N)
            cr:show_text (i)
            cr:stroke()
            
            for j, k in pairs (Comp.stlist[i].transition) do
                count (i, k.Destination, Comp.STdx[i], Comp.STdy[i], Comp.STdx[k.Destination], Comp.STdy[k.Destination])
                --print('from ', i, 'to', k.Destination, 'if ', k.Condition)
            end
            
            
        end
        cr:move_to (50*N, 236*N )
        cr:select_font_face('sans', cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_BOLD)
        cr:set_font_size(16*N)
        cr:show_text('Transitions:')
        for i, _ in pairs(Comp.stlist) do
            for j, k in pairs (Comp.stlist[i].transition) do
                connect (i, k.Destination, Comp.STdx[i], Comp.STdy[i], Comp.STdx[k.Destination], Comp.STdy[k.Destination], k.Condition)
                --print('from ', i, 'to', k.Destination, 'if ', k.Condition)
            end
        end    
    end
    ...
    local ZoomIn = function()
        N = N + 0.05
        area:set_size_request(area:get_allocated_size()*(1.05), area:get_allocated_size()*(1.05))
        for i, v in ipairs(Origem) do
            v[1] = v[1] - 5
        end
        x = x - 5
    ...
    local ZoomOut = function()
        N = N - 0.05
        area:set_size_request(area:get_allocated_size()*(0.95), area:get_allocated_size()*(0.95))
        for i, v in ipairs(Origem) do
            v[1] = v[1] + 5
        end
        x = x + 5
    ...
    local GoForward = function()
        ...
        local txt = combo:get_active_text()
        if txt then
            if radio1:get_active() and Comp.Class =='Comp' then
                draw_type = 'ECC'
            elseif radio1:get_active() and not Comp.Class then
                draw_type = 'network'
            elseif radio2:get_active() then
                draw_type = 'block'
            end
    ...
    local Reset = function()
        area:set_size_request(1200, 1200)
        for i, v in ipairs(Origem) do
            v[1] = v[1] - x
            v[2] = v[2] - y
        end
    ...
    local function RADIO()
        if radio2:get_active() then
            draw_type = 'block'
        elseif radio1:get_active() and Comp.Class =='Comp' then
            draw_type = 'network'
        elseif radio1:get_active() and Comp.Class =='Basic' then
            draw_type = 'ECC'
        elseif radio1:get_active() then
            draw_type = 'network'
        end
        area:queue_draw()
    end
    ...
    toolbar:add(toolbutton1, toolbutton2, sep1, toolbutton3, toolbutton4, --[[toolbutton5, toolbutton6,]] sep2, radio2, radio1, sep3, toolbutton7)
    hbox1:pack_start(toolbar, true, true, 0)
    paned:add1(sidebox)
    scroll:add_with_viewport(hbox2)
    paned:add2(scroll)
    hbox2:pack_start(area, true, true , 0)
    main_box:pack_start(hbox1, false, false, 0)
    main_box:pack_end(paned, true, true, 0)
    window:set('default-width', 1000, 'default-height', 650)
    --window:add(toolbar)
    window:add(main_box)
    window:set('title', "GASR FB-Viewer v.1.0", 'window-position', gtk.WIN_POS_CENTER)
    window:show_all()
