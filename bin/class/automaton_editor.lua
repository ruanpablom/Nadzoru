...
AutomatonEditor = letk.Class( function( self, gui, automaton )
    ...
    self.vbox                  = gtk.Box.new(gtk.ORIENTATION_VERTICAL, 0)
		self.toolbar           = {gtk.Toolbar.new(), gtk.Toolbar.new()}
    ...
    --self.treeview_events:add_column_toggle("Con", 50, self.toggle_controllable, self ) --REMOVE
    --self.treeview_events:add_column_toggle("Obs", 50, self.toggle_observable,  self ) --REMOVE

    self.vbox:pack_start( self.toolbar[1], false, false, 0 )
    self.vbox:pack_start( self.toolbar[2], false, false, 0 )
    self.vbox:pack_start( self.hbox, true, true, 0 )
        ...
        self.hbox:pack_start( self.vbox2, false, false, 0 )
			...
			--self.vbox2:pack_start( gtk.Label.new_with_mnemonic('Level:'), false, false, 0 )
			--self.vbox2:pack_start( self.level_box, false, false, 0 )
			...
			--self.treeview_events.columns[2]:set('sizing', gtk.TREE_VIEW_COLUMN_AUTOSIZE) --REMOVE
			--self.treeview_events.render[2]:set('width', 32) --REMOVE
    ...
    self.toolbar[1]:insert( self.btn_act_save, -1 )				
    ...
    self.toolbar[1]:insert( self.btn_act_saveas, -1 )
    ...
    self.toolbar[1]:insert( self.btn_act_idesexport, -1 )
    ...
    self.toolbar[1]:insert( self.btn_act_tctexport, -1 )
    ...
    self.toolbar[1]:insert( self.btn_act_png, -1 )

    --separator
    self.sep = gtk.SeparatorToolItem.new()
    self.toolbar[1]:insert( self.sep, -1 )

    --undo
    self.img_act_undo = gtk.Image.new_from_stock( 'gtk-go-back' )
    ...
    self.toolbar[1]:insert( self.btn_act_undo, -1 )

    --redo
    self.img_act_redo = gtk.Image.new_from_stock( 'gtk-go-forward' )
    ...
    self.toolbar[1]:insert( self.btn_act_redo, -1 )
    ...
    self.toolbar[2]:insert( self.btn_act_edit, -1 )
    ...
    self.toolbar[2]:insert( self.btn_act_move, -1 )
    ...
    self.toolbar[2]:insert( self.btn_act_move_motion, -1 )
    ...
    self.toolbar[2]:insert( self.btn_act_state, -1 )
	...
	self.toolbar[2]:insert( self.btn_act_state_n, -1 )
	...
	self.toolbar[2]:insert( self.btn_act_state_y, -1 )
    ...
    self.toolbar[2]:insert( self.btn_act_initial, -1 )
    ...
    self.toolbar[2]:insert( self.btn_act_marked, -1 )
    ...
    self.toolbar[2]:insert( self.btn_act_transition, -1 )
    ...
    self.toolbar[2]:insert( self.btn_act_delete, -1 )
    ...
    self.toolbar[2]:insert( self.btn_act_positionstates, -1 )
...
function AutomatonEditor:update_treeview_events(hist)
    ...
    for event_id, event in self.automaton.events:ipairs() do
        ...
        --self.treeview_events:add_row{ event.name, event.controllable, event.observable } --REMOVE
    end
...
function AutomatonEditor:edit_transition( source, target )
    ...
    btnOk:connect('clicked', function()
		for k_event, toggle in ipairs(events_toggle) do
			if toggle and not transitions_map[k_event] then
				self.automaton:transition_add(source, target, events_map[k_event])
			end
			if not toggle and transitions_map[k_event] then
